...
import { getAIInsights } from "@/lib/actions/getAIInsights";
import { generateInsightAnswer } from "@/lib/actions/generateInsightAnswer";
...
const AIInsights = () => {
  ...
  const getButtonColors = (type: string) => {
    switch (type) {
      ...
      case "info":
        return "text-emerald-700  hover:text-emerald-800 ";
      default:
        return "text-gray-700 dark:text-gray-300 hover:text-gray-800 ";
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="bg-white/80 p-4 sm:p-6 rounded-2xl shadow-xl mt-8">
        <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
          ...
          <div className="flex-1">
            <h3 className="text-lg sm:text-xl font-bold text-gray-90">
              AI Insights
            </h3>
        ...
        <div className="space-y-3 sm:space-y-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="animate-pulse bg-gradient-to-r from-gray-100 to-gray-50p-3 sm:p-4 rounded-xl border border-gray-100"
            >
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm p-4 sm:p-6 rounded-2xl shadow-lg hover:shadow-xl mt-8">
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div>
            <h3 className="text-lg sm:text-xl font-bold">AI Insights</h3>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="inline-flex items-center gap-2 bg-dark px-2 py-1 rounded-full text-xs font-medium">
          ...
          <button
            ...
            className="w-7 h-7 sm:w-8 sm:h-8 bg-dark text-white rounded-xl flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-200"
            disabled={isLoading}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
        {insights.map((insight) => {
          ...
          return (
            <div
              key={insight.id}
              className={`relative overflow-hidden rounded-xl p-3 sm:p-4 border-l-4 transition-all duration-200 ${getInsightColors(
                insight.type
              )}`}
            >
                        ...
                        <div className="flex-1">
                          ...
                          {currentAnswer.isLoading ? (
                            <div className="space-y-1">
                              <div className="animate-pulse bg-gray-200 h-2 rounded-lg w-full"></div>
                              <div className="animate-pulse bg-gray-200 h-2 rounded-lg w-3/4"></div>
                              <div className="animate-pulse bg-gray-200 h-2 rounded-lg w-1/2"></div>
                            </div>
                          ) : (
      ...
      <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-100">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-0">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            <div className="w-5 h-5 sm:w-6 sm:h-6 bg-dark rounded-lg flex items-center justify-center">
              <span className="text-sm">🧠</span>
            </div>
          ...
          <button
            onClick={loadInsights}
            className="px-3 py-1.5 bg-dark text-white rounded-lg font-medium text-xs shadow-lg hover:shadow-xl transition-all duration-200"
          >
